...
struct AliFlashClient {
    ...
    ~AliFlashClient() {
...
#ifdef ALIFLASH_DEBUG_CAL
        {
            std::cout << "Aliflash called number: " << cal_info.call_num
                      ...
                      << ", average batch cal number: "
                      << cal_info.cal_num / (cal_info.call_num + 0.00001f) << std::endl;
        }
#endif
    }
    ...
    void
    upload(float* base_data, size_t vecsize) {
#ifdef MULTICASE
        static bool init = false;
        if (init)
            return;
        ...
        std::cout << "Upload ready, "
                  << "vecsize: " << vecsize_ << ", vecdim: " << vecdim_ << std::endl;
        ...
        for (i = 0; i + offset <= vecsize_; i += offset) {
            ...
            ret = pnmesdk_db_storage(
                context_, (char*)(base_data + i * vecdim_), offset * vecdim_ * FP32);
        }
        ...
        if (i < vecsize_) {
            ...
            ret = pnmesdk_db_storage(
                context_, (char*)(base_data + i * vecdim_), (vecsize_ - i) * vecdim_ * FP32);
        }

        if (ret) {
            std::cout << "pnmesdk_db_storage failed, [data upload failed] ret = " << ret
                      << std::endl;
            exit(1);
        }
