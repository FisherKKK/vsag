...
uint32_t
FlashSearcher::visit(const GraphInterfacePtr& graph,
...
FlashSearcher::visit(const GraphInterfacePtr& graph,
                     const VisitedListPtr& vl,
                     const Vector<InnerIdType>& current_node_pairs,
                     const FilterPtr& filter,
                     float skip_ratio,
                     Vector<InnerIdType>& to_be_visited_rid,
                     Vector<InnerIdType>& to_be_visited_id,
    ...
    auto look_ahead_neighbors = [&]() {
        InnerIdType cur{0};
        for (auto current_node : current_node_pairs) {
            Vector<InnerIdType> current_neighbor(graph->MaximumDegree(), allocator_);
            graph->GetNeighbors(current_node, current_neighbor);
            auto cur_neighbor_size = current_neighbor.size();

            memcpy(neighbors.data() + cur,
                   current_neighbor.data(),
                   cur_neighbor_size * sizeof(InnerIdType));
            cur += cur_neighbor_size;
        }
    };

    auto look_ahead_neighbors_locked = [&]() {
        InnerIdType cur{0};
        for (auto current_node : current_node_pairs) {
            SharedLock lk(this->mutex_array_, current_node);
    ...
    if (this->mutex_array_ != nullptr) {
        look_ahead_neighbors_locked();
    } else {
        look_ahead_neighbors();
    }
...
MaxHeap
FlashSearcher::Search(const GraphInterfacePtr& graph,
...
MaxHeap
FlashSearcher::search_impl(const GraphInterfacePtr& graph,
                           const FlattenInterfacePtr& flatten,
                           const VisitedListPtr& vl,
                           const float* query,
                           const InnerSearchParam& inner_search_param,
...
FlashSearcher::search_impl(const GraphInterfacePtr& graph,
    ...
    uint32_t dist_cmp = 0;
    uint32_t count_no_visited = 0;
    Vector<InnerIdType> to_be_visited_rid(graph->MaximumDegree() * look_ahead_, allocator_);
    Vector<InnerIdType> to_be_visited_id(graph->MaximumDegree() * look_ahead_, allocator_);
    Vector<InnerIdType> neighbors(graph->MaximumDegree() * look_ahead_, allocator_);
